...
test("dump target cell full", async () => {
  ...
  const { setGetMapImageSize, mapdata_cell, pl_mpos } =
    await import("../src/lib/mapdata");
  const { initCommands } = await import("../src/lib/p_cmd");
  const { resetReplaySandboxState, toPacketBuffer } =
  ...
  const lines = readFileSync(
    "/tmp/workspace/bofh69/crossfire-web-client/tests/replay-mapdata/logs/zoo.log",
    "utf8",
  ).split(/\r?\n/);
  let absX = 0,
    absY = 0;
  for (let i = 0; i < 1830; i++) {
    ...
    if (i === 1827 || i === 1828) {
      ...
      const tails = c.tails
        ...
        .filter((t) => t.face !== 0 || t.sizeX !== 0 || t.sizeY !== 0);
      const smooth = c.smooth
        .map((v, j) => ({ layer: j, value: v }))
        .filter((s) => s.value !== 0);
      console.log(
        "after line",
        i + 1,

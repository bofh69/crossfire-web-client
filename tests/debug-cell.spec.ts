...
describe("debug3", () => {
  test("step by entry", () => {
    const logText = readFileSync(
      "tests/replay-mapdata/logs/big-face-in-scorn.log",
      "utf8",
    );
    const parsed = parseReplayLogFile(logText);

    const afterUpdateMark = parsed.marks.find(
      (m: any) => m.label === "after update",
    )!;
    const fromTailMark = parsed.marks.find(
      (m: any) => m.label === "from tail to head",
    )!;
    ...
    for (
      let i = afterUpdateMark.entryIndex + 1;
      i <= fromTailMark.entryIndex;
      i++
    ) {
      const entry = parsed.entries[i]!;

...
import {
  updateHotbarFacesFromSpell,
  updateHotbarSlotFromItem,
  resetHotbarSession,
...
export function PlayerCmd(data: DataView, len: number): void {
  ...
  if (reader.pos !== len) {
    LOG(
      LogLevel.Warning,
      "PlayerCmd",
      `lengths do not match (${reader.pos}!=${len})`,
    );
  }
...
export function Item2Cmd(data: DataView, len: number): void {
  ...
  while (reader.remaining > 0) {
    ...
    if (playerTag && location === playerTag) {
      ...
      if (sName)
        hotbarChanged =
          updateHotbarSlotFromItem(sName, tag, face) || hotbarChanged;
    }
  }
  gameEvents.emit("playerUpdate");
...
export function UpdateItemCmd(data: DataView, len: number): void {
  ...
  if (reader.pos > len) {
    LOG(
      LogLevel.Warning,
      "UpdateItemCmd",
      `Overread buffer: ${reader.pos} > ${len}`,
    );
    return;
  }
...
export function UpdspellCmd(data: DataView, len: number): void {
  ...
  if (flags & UPD_SP_MANA) {
    spell.sp = reader.readInt16();
  }
  if (flags & UPD_SP_GRACE) {
    spell.grace = reader.readInt16();
  }
  if (flags & UPD_SP_DAMAGE) {
    spell.dam = reader.readInt16();
  }
  gameEvents.emit("spellUpdate");
}

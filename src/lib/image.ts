...
export function registerMagicMapFaces(): void {
  ...
  function drawSegment(x1: number, y1: number, x2: number, y2: number): void {
...
export function getImageInfo(data: Uint8Array, len: number): void {
  ...
  for (let i = 2; i < lines.length; i++) {
    ...
    if (setnum < 0 || setnum >= MAX_FACE_SETS) {
      LOG(LogLevel.Warning, "getImageInfo", `setnum out of range: ${setnum}`);
      continue;
    }
...
export function image_debug_face(face: number): string[] {
  ...
  const { w, h } = getMapImageSize(face);
  lines.push(
    `  pixel size: ${w}×${h}${hasSizeEntry ? "" : " (default – image not loaded)"}`,
  );

  const url = getFaceUrl(face);
  lines.push(
    `  image URL: ${url !== null ? url : "(none – image not loaded)"}`,
  );

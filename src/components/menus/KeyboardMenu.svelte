<script lang="ts">
  ...
  function handleBindCmdKeyCapture(e: KeyboardEvent) {
    ...
    dialogKeyStr = keyEventToString(e);
    const extraFlags =
      (dialogBindCmdAny ? KEYF_ANY : 0) | (dialogBindCmdEdit ? KEYF_EDIT : 0);
...
{:else if dialogMode === "bind-confirm"}
  <div class="dialog-overlay">
    <div class="dialog">
      ...
      {#if dialogExisting}
        <p class="dialog-warn">
          Already bound to: <strong>{dialogExisting.command}</strong>
        </p>
        <p>Overwrite?</p>
      {/if}
      <div class="dialog-buttons">
    ...
    <div class="dialog dialog-widest">
      ...
      <div class="dialog-buttons">
        {#if pendingKeyScopes.size > 0}
          <button class="btn-primary" onclick={saveKeyBindingScopes}
    ...
    <div class="dialog">
      ...
      <div class="dialog-buttons">
        <button
          ...
          onclick={beginBindCmdKeyCapture}>Capture Key…</button
        >
...
{:else if dialogMode === "bind-cmd-hotbar-input"}
  <div class="dialog-overlay">
    <div class="dialog">
      ...
      <label class="dialog-field-label" for="bind-cmd-hotbar-cmd"
        >Command:</label
      >
      <!-- svelte-ignore a11y_autofocus -->
      <input
        id="bind-cmd-hotbar-cmd"
        class="dialog-input"
        type="text"
        bind:value={dialogCommand}
        autofocus
      />
      <label class="dialog-field-label" for="bind-cmd-hotbar-label"
        >Label (leave blank to use command):</label
      >
      <input
        id="bind-cmd-hotbar-label"
        class="dialog-input"
        type="text"
        bind:value={dialogHotbarLabel}
        placeholder={dialogCommand}
      />
      <label class="dialog-field-label" for="bind-cmd-hotbar-slot">Slot:</label>
      <select
        id="bind-cmd-hotbar-slot"
        class="dialog-input dialog-select"
        bind:value={dialogHotbarSlot}
      >
      ...
      <div class="dialog-buttons">
        <button
          class="btn-primary"
          disabled={!dialogCommand}
          onclick={handleBindCmdHotbar}>Bind</button
        >
  ...
  <div class="dialog-overlay">
    <div class="dialog">
      ...
      {#if hotbarExistingSlot}
        <p class="dialog-warn">
          Slot already assigned to: <strong>{hotbarExistingSlot.label}</strong>
        </p>
        <p>Overwrite?</p>
      {/if}
      <div class="dialog-buttons">
        <button class="btn-primary" onclick={confirmBindCmdHotbar}
          >Overwrite</button
        >
